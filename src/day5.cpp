...
public:
  Range(std::uint64_t ds, std::uint64_t ss, std::uint64_t rl)
  ...
  void update_ds(std::uint64_t lb) { destination_start = lb; }

  void update_ss(std::uint64_t lb) { source_start = lb; }

  void update_width(std::uint64_t w) { width = w; }
};

std::ostream&
operator<<(std::ostream& os, const Range& r)
{
  os << r.lbi() << " " << r.lbi() + r.w() << " -> " << r.lbo() << " "
     << r.lbo() + r.w();
  return os;
}
...
bool
Range::contains(std::uint64_t num)
{
  return num >= source_start && num < source_start + width;
}

std::uint64_t
Range::map(std::uint64_t num)
{
...
Overlap
calc_overlap(const Range& r1, const Range& r2)
{
  auto r1_lb = r1.lbo();
...
void
fill_out_range(std::vector<Range>& r_vec, std::uint64_t max_seed)
{
  ...
  while (start < max_seed) {
    if (ind < r_vec.size()) {
      std::uint64_t delta =
        r_vec[ind].lbi() > start ? r_vec[ind].lbi() - start : 0;
      if (0 == delta) {
...
range_collapse(std::vector<std::vector<Range>>& stacked_filter,
      ...
      for (Range& r2 : stacked_filter[i]) {
        ...
        switch (calc_overlap(r1, r2)) {
          ...
          case Overlap::covers:
            ...
            queue.emplace_back(
              r1.lbo() + delta, r1.lbi() + delta, r1.w() - delta);
            r1.update_ds(r2.lbo());
...
problem_1(std::vector<std::string>& input)
{
  ...
  for (std::string const& row : input | std::views::drop(3)) {
    ...
    std::uint64_t widest =
      dest_start > source_start ? dest_start + width : source_start + width;
...
problem_2(std::vector<std::string>& input)
{
  ...
  std::ranges::sort(flat_filter,
                    [](Range& r1, Range& r2) { return r1.lbi() < r2.lbi(); });
...
main()
{
  ...
  std::vector<std::string> input =
    help_func::read_input("../data/input-day-5.txt");
