...
bool all_zero(std::vector<std::int64_t> &vec) {
...
void problem_1_thread(const std::vector<std::string> &input, const std::size_t n_row, const std::size_t n_threads, const std::size_t index) {
    
    std::int64_t sum{};
    std::size_t upper_bound = (index == n_threads - 1) ? input.size() : (index + 1)*n_row;
...
void problem_1(const std::vector<std::string>& input, const std::size_t n_threads){
    ...
    {
        thread_vec.emplace_back(problem_1_thread, std::ref(input), n_rows_per_thread, n_threads, i);
    }
...
void problem_2_thread(const std::vector<std::string>& input, const std::size_t n_row, const std::size_t n_threads, const std::size_t index) {
    std::int64_t val{};
    std::size_t upper_bound = (index == n_threads - 1) ? input.size() : (index + 1)*n_row;
...
void problem_2(const std::vector<std::string>& input, std::size_t n_threads){
    ...
    for(std::size_t i = 0; i < n_threads; ++i)
    {
        thread_vec.emplace_back(problem_2_thread, std::ref(input), n_rows_per_thread, n_threads, i);
    }
