...
std::size_t calc_distance(const std::pair<std::size_t, std::size_t> &g1, const std::pair<std::size_t, std::size_t> &g2, const std::vector<bool> &cols_ptr, const std::vector<bool> &rows_ptr) {
    ...
    std::uint64_t distance = high_y - low_y + high_x - low_x + open_cols + open_rows;

    return distance;
}

std::size_t calc_distance_2(const std::pair<std::size_t, std::size_t> &g1, const std::pair<std::size_t, std::size_t> &g2, const std::vector<bool> &cols_ptr, const std::vector<bool> &rows_ptr) {
    std::uint64_t low_x = (g1.first < g2.first) ? g1.first : g2.first;
    std::uint64_t high_x = (g1.first > g2.first) ? g1.first : g2.first;
    ...
    std::uint64_t distance = high_y - low_y + high_x - low_x + open_cols*999999 + open_rows*999999;
...
void calculation_thread(
    ...
    std::size_t upper_bound = (index+1)*d_size > i_vec.size() ? i_vec.size() : (index+1)*d_size;

    std::size_t local_sum{};
    for(std::size_t i = index*d_size; i < upper_bound; ++i) {
        local_sum += calc_distance(g_vec[i_vec[i].first],g_vec[i_vec[i].second], cols_ptr, rows_ptr);
    }
    {
        std::lock_guard<std::mutex> lg(t_sum_1_mutex);
        // std::cout << "My index is " << index << " and I processed the pairs between " << index*d_size << " and " << upper_bound << "\n";
        t_sum_1 += local_sum;
    }
}


void calculation_thread_2(
                        const std::vector<std::pair<std::size_t, std::size_t>> &g_vec, 
                        const std::vector<std::pair<std::size_t, std::size_t>> &i_vec,
                        std::size_t index, 
                        std::size_t n_threads, 
    ...
    for(std::size_t i = index*d_size; i < upper_bound; ++i) {
        local_sum += calc_distance_2(g_vec[i_vec[i].first],g_vec[i_vec[i].second], cols_ptr, rows_ptr);
    }
    {
...
void problem_1(const std::vector<std::string>& input, std::size_t n_threads){
    ...
    for(std::size_t i = 0; i < n_threads; ++i) {
        thread_vec.emplace_back(calculation_thread, std::ref(galaxy_coords), std::ref(pair_vec), i, n_threads, n_data_points, std::ref(occupied_cols), std::ref(occupied_rows));
    }
...
void problem_2(const std::vector<std::string>& input, std::size_t n_threads){
    ...
    for(std::size_t i = 0; i < n_threads; ++i) {
        thread_vec.emplace_back(calculation_thread_2, std::ref(galaxy_coords), std::ref(pair_vec), i, n_threads, n_data_points, std::ref(occupied_cols), std::ref(occupied_rows));
    }
...
int main(){
    help_func::Timer t;
    std::vector<std::string> input =help_func::read_input("../data/input-day-11.txt");
