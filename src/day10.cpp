...
void find_start_loc(const std::vector<std::string>& input) {
...
inline bool expand_up(const std::pair<std::size_t, std::size_t> &loc, const std::vector<std::string>& input) {
    if(loc.first == 0) {
        return false;
    }
    return input[loc.first - 1][loc.second] == 'F' || input[loc.first - 1][loc.second] == '7' || input[loc.first - 1][loc.second] == '|';
}

inline bool expand_right(const std::pair<std::size_t, std::size_t> &loc, const std::vector<std::string>& input) {
    if(loc.second + 1 == input[loc.first].size()) {
        return false;
    }
    return input[loc.first][loc.second + 1] == 'J' || input[loc.first][loc.second + 1] == '7'|| input[loc.first][loc.second + 1] == '-';
}

inline bool expand_down(const std::pair<std::size_t, std::size_t> &loc, const std::vector<std::string>& input) {
    if(loc.first + 1 == input.size()) {
        return false;
    }
    return input[loc.first+ 1][loc.second] == 'J' || input[loc.first + 1][loc.second] == 'L' || input[loc.first + 1][loc.second] == '|';
}

inline bool expand_left(const std::pair<std::size_t, std::size_t> &loc, const std::vector<std::string>& input) {
    if(loc.second == 0) {
        return false;
    }
    return input[loc.first][loc.second - 1] == 'F' || input[loc.first][loc.second - 1] == 'L'|| input[loc.first][loc.second - 1] == '-';
}


void problem_1(const std::vector<std::string>& input){
    while(!loc_found.load(std::memory_order_relaxed)) {
        __builtin_ia32_pause();
    }
    std::pair<std::size_t, std::size_t> start_loc{X.load(std::memory_order_relaxed), Y.load(std::memory_order_relaxed)};
    ...
    while(!queue.empty()){
        ...
        case 'S':
            if(expand_up(current_elem, input)) {
                queue.emplace_back(std::make_pair(current_elem.first - 1, current_elem.second), distance + 1);
            }

            if(expand_right(current_elem, input)) {
                queue.emplace_back(std::make_pair(current_elem.first, current_elem.second + 1), distance + 1);
            }
...
inline bool vert_crossing(const char &c) {
    return '7' == c || 'F' == c || '|' == c;
}

void problem_2(const std::vector<std::string>& input){
    while(!loc_found.load(std::memory_order_relaxed)) {
        __builtin_ia32_pause();
    }
    std::pair<std::size_t, std::size_t> start_loc{X.load(std::memory_order_relaxed), Y.load(std::memory_order_relaxed)};
