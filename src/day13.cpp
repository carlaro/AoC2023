...
public:
  input_queue(const std::vector<T>& i, std::mutex& mutex)
    : m_input(i)
    , m_mutex(mutex)
    , m_it(m_input.begin())
    , m_end(m_input.end())
...
std::uint64_t
find_reflected_rows(std::vector<std::string>& pattern)
{
  ...
  if (viable_horizontal_positions.any()) {
    size_t horizontal_result =
      std::countr_zero(viable_horizontal_positions.to_ulong()) + 1;
  ...
  if (viable_vertical_positions.any()) {
    size_t vertical_result =
      std::countr_zero(viable_vertical_positions.to_ulong()) + 1;
...
void
problem_1_thread(input_queue<std::vector<std::string>>& iq)
{
...
bool
is_power_of_two(std::uint64_t num)
{
  return (num & (num - 1)) == 0;
}

bool
check_horizontal(size_t row,
                 size_t col,
                 size_t steps,
                 bool& smudge_fixed,
...
std::uint64_t
find_reflected_smudged_rows(std::vector<std::string>& pattern)
{
...
void
problem_2_thread(input_queue<std::vector<std::string>>& iq)
{
  std::uint64_t local_sum{};
  while (iq.ready()) {
    std::vector<std::string> pattern = iq.get();
...
void
problem_1(const std::vector<std::vector<std::string>>& input,
...
void
problem_2(const std::vector<std::vector<std::string>>& input,
          std::size_t n_threads)
{
  input_queue<std::vector<std::string>> iq(input, input_mutex);
  std::vector<std::thread> thread_vec;
...
main()
{
  ...
  std::vector<std::string> lines =
    help_func::read_input("../data/input-day-13.txt");
  // std::vector<std::string> lines =
  // help_func::read_input("../data/small_data.txt");
