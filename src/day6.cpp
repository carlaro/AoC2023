...
[[nodiscard]] int
calc_valid_strats(double time, double distance)
{
  if (time * time - 4.0 * distance < 0) {
    return 0;
  }
  double upper_bound =
    std::floor(time * 0.5 + std::sqrt(time * time * 0.25 - distance));
  double lower_bound =
    std::ceil(time * 0.5 - std::sqrt(time * time * 0.25 - distance));
...
main()
{
  ...
  std::vector<std::string> input =
    help_func::read_input("../data/input-day-6.txt");
