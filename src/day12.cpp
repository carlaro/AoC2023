...
class input_queue {
    ...
    public:
        input_queue(const std::vector<std::string> &i, std::mutex& mutex) : m_input(i), m_mutex(mutex), m_it(m_input.begin()), m_end(m_input.end()), m_num(m_input.size()) {}
...
std::uint64_t possible_assignment(const std::string &s, std::vector<int> t, size_t t_ind, size_t s_ind, int count, int blocks_started)
{
    while (s_ind < s.size()) {
        if (s[s_ind] == '.') {
            if (count == 0) {
        ...
        if (s[s_ind] == '?') {
            ...
            s1[s_ind] = '#';
            std::uint64_t ans_left = possible_assignment(s1, t, t_ind, s_ind, count, blocks_started);
            std::string s2 = std::string(s);
            s2[s_ind] = '.';
            std::uint64_t ans_right = possible_assignment(s2, t, t_ind, s_ind, count, blocks_started);
...
bool check_if_group_fits(const std::string &record, int group, size_t from) {
...
std::uint64_t possible_assignment_2(const std::string &record, std::vector<int> groups, std::unordered_map<std::string, std::uint64_t>& hashmap) {
    ...
    if (record[record_index] == '#') {
        ...
        if(fits) {
            ...
            std::vector<int> remaining_groups = std::vector<int>(groups.begin() + 1, groups.end());
            std::uint64_t lval = possible_assignment_2(remaining_record, remaining_groups, hashmap);
            hashmap[key] = lval;
...
void problem_1_thread(input_queue& iq)
{
    ...
    while(iq.ready())
    {
        ...
        std::uint64_t answer = possible_assignment(string_vec[0], connected_broken_springs, 0, 0, 0, 0);
        local_sum += answer;
    }
    {
...
void problem_2_thread(input_queue& iq)
{
    ...
    while(iq.ready())
    {
        ...
        auto string_vec = help_func::split(s, " ");
        std::string repeated_string = help_func::repeat_string(string_vec[0], 5, "?");
        ...
        std::uint64_t score = possible_assignment_2(repeated_string, connected_broken_springs, local_map);
        local_sum += score;
        ++line_count;
    }
    {
...
{
    help_func::Timer t;
    std::vector<std::string> input = help_func::read_input("../data/input-day-12.txt");
