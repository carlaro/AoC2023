...
[[nodiscard]] std::vector<std::string>
split(const std::string& s, std::string delim, bool trim_empty = true)
{
...
join(const std::string& s1, const std::string& s2, std::string delim = "")
{
  std::string ret;
...
[[nodiscard]] std::string
repeat_string(const std::string& s,
              std::size_t times,
              std::string padding = "",
              bool skip_last_pad = true)
{
  std::string ret;
...
{
  s.erase(
    std::find_if(s.rbegin(), s.rend(), [](auto c) { return !std::isspace(c); })
      .base(),
    s.end());
}
...
void
ltrim_char(std::string& s, char c)
{
  s.erase(s.begin(),
          std::find_if(s.begin(), s.end(), [c](auto c1) { return c != c1; }));
}

void
rtrim_char(std::string& s, char c)
{
  s.erase(
    std::find_if(s.rbegin(), s.rend(), [c](auto c1) { return c != c1; }).base(),
    s.end());
}
